...
class SmartOutfitRecommender:
    ...
    def _build_color_priority_outfits(self, occasions, context, required, forbidden):
        """Try to build outfits using only items that match the requested color(s)."""
        color_reqs = [r for r in required if r in self.color_variants]
        if not color_reqs:
            return None
        color_variants = set()
        ...
        for item in occasion_items:
            ...
            if any(color in tags or color in name for color in color_variants):
                color_matched.append(item)
        if not color_matched:
            return None
        temp_recommender = SmartOutfitRecommender(color_matched)
        non_color_required = [r for r in required if r not in color_reqs]
        outfits = temp_recommender.get_unique_outfits(occasions, context, non_color_required, forbidden)
        return outfits if outfits else None
