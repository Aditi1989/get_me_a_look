...
class SmartOutfitRecommender:
    ...
    def precompute_all_possible_combos(self):
        ...
        layers = [item for item in self.wardrobe_db if item["category"] == "layer"]
                ...
                if occasion in top.get("tags", []):
                    for bottom in bottoms:
                        if occasion in bottom.get("tags", []):
                            ...
                            # Top+bottom+layer if layer is appropriate
                            if "+layer" in self.occasion_outfit_types[occasion]:
                                for layer in layers:
                                    if occasion in layer.get("tags", []):
                                        combos[occasion].append((top["id"], bottom["id"], layer["id"]))
    ...
    def find_matching_items(self, required_kws, preferred_kws, forbidden_kws, context, occasion):
        ...
        for item in items:
            ...
            if score > 0:  # Only consider items with positive scores
                item_copy = dict(item)
    ...
    def assemble_outfits(self, matched_items, required_kws, preferred_kws, forbidden_kws, context, occasion):
        outfits = []
        context_key = f"{occasion}_{context['time']}_{context['weather']}"
        shown_combos = self.shown_combos_per_context.get(context_key, set())
        
        # Prepare category lists with occasion-specific filtering
        ...
        # Strict filtering for formal/office/business/interview
        if occasion in ["office", "business_meeting", "interview"]:
            # Only allow items that are strictly formal/office/professional
            tops = [t for t in tops if any(tag in t.get("tags", []) for tag in ["formal", "office", "professional"])]
            bottoms = [b for b in bottoms if any(tag in b.get("tags", []) for tag in ["formal", "office", "professional"])]
            layers = [l for l in layers if any(tag in l.get("tags", []) for tag in ["formal", "office", "professional"])]
            # No one-pieces for these occasions
            one_pieces = []
        elif occasion in ["party", "wedding", "date", "office_party"]:
            # Allow a broader set of tops for party/wedding/date/office_party
            fancy_tags = {"fancy", "elegant", "stylish", "fusion", "modern"}
            tops = [t for t in tops if any(tag in t.get("tags", []) for tag in fancy_tags)]
            bottoms = [b for b in bottoms if any(tag in b.get("tags", []) for tag in fancy_tags)]
            layers = [l for l in layers if any(tag in l.get("tags", []) for tag in fancy_tags)]
            one_pieces = [op for op in one_pieces if any(tag in op.get("tags", []) for tag in fancy_tags)]
        elif occasion == "ritual":
            tops = [t for t in tops if "ethnic" in t.get("tags", []) or "traditional" in t.get("tags", [])]
            bottoms = [b for b in bottoms if "ethnic" in b.get("tags", []) or "traditional" in b.get("tags", [])]

        # For special occasions, always include at least one one-piece
        if occasion in ["party", "wedding", "date", "office_party"] and one_pieces:
            # Always suggest a one-piece as the first outfit if available, and rotate through all one-pieces before repeating
            one_piece_history_key = f"{occasion}_one_piece_history"
            if not hasattr(self, "one_piece_history"):
                self.one_piece_history = defaultdict(list)
            used_one_pieces = self.one_piece_history[one_piece_history_key]
            available_one_pieces = [op for op in sorted(one_pieces, key=lambda x: x["score"], reverse=True) if op["id"] not in used_one_pieces]
            if not available_one_pieces:
                # Reset history if all have been used
                self.one_piece_history[one_piece_history_key] = []
                available_one_pieces = sorted(one_pieces, key=lambda x: x["score"], reverse=True)
            for op in available_one_pieces:
                combo_id = (op["id"],)
                if combo_id not in shown_combos:
                    ...
                    self.one_piece_history[one_piece_history_key].append(op["id"])
                    break  # Only one one-piece per batch

        # Generate top+bottom combinations with maximum variety and unique tops
        if tops and bottoms and len(outfits) < 3:
            # For party/date/wedding/office_party: only use tops/bottoms with at least one of the fancy tags and NOT formal/office/professional
            if occasion in ["party", "wedding", "date", "office_party"]:
                fancy_tags = {"fancy", "elegant", "stylish", "fusion", "modern"}
                tops_filtered = [
                    t for t in tops
                    if any(tag in t.get("tags", []) for tag in fancy_tags)
                    and not any(tag in t.get("tags", []) for tag in ["formal", "office", "professional"])
                ]
                bottoms_filtered = [
                    b for b in bottoms
                    if any(tag in b.get("tags", []) for tag in fancy_tags)
                    and not any(tag in b.get("tags", []) for tag in ["formal", "office", "professional"])
                ]
                # Track used tops globally for this occasion to avoid repeats until all are used
                top_history_key = f"{occasion}_top_history"
                if not hasattr(self, "top_history"):
                    self.top_history = defaultdict(list)
                used_tops_history = self.top_history[top_history_key]
                available_tops = [t for t in tops_filtered if t["id"] not in used_tops_history]
                if not available_tops:
                    self.top_history[top_history_key] = []
                    available_tops = tops_filtered
                random.shuffle(available_tops)
                tops_sorted = available_tops
            else:
                tops_sorted = sorted(tops, key=lambda x: x["score"], reverse=True)
                bottoms_filtered = bottoms

            bottoms_sorted = sorted(bottoms_filtered, key=lambda x: x["score"], reverse=True)
            random.shuffle(bottoms_sorted[:8])  # Shuffle top 8 bottoms

            used_tops = set()
            used_bottoms = set()
            combos_added = 0

            for top in tops_sorted:
                if len(outfits) >= 3:
                    break
                if top["id"] in used_tops:
                    continue

                for bottom in bottoms_sorted:
                    if bottom["id"] in used_bottoms:
                        continue
                    combo_id = (top["id"], bottom["id"])
                    if combo_id in shown_combos:
                        continue
                    ...
                    # Add layer if needed/requested and available
                    if (context["needs_layer"] or "layer" in required_kws) and layers:
                        best_layer = None
                        best_score = -1
                        for layer in layers:
                            layer_combo_id = (top["id"], bottom["id"], layer["id"])
                            if layer_combo_id not in shown_combos:
                                layer_score = self.score_item(layer, required_kws, preferred_kws, forbidden_kws, context, occasion)
                        ...
                        if best_layer:
                            ...
                            outfit_type += "+layer"
                            combo_id = (top["id"], bottom["id"], best_layer["id"])

                    outfits.append({
                        "type": outfit_type,
                        "items": outfit_items,
                        "score": sum(item["score"] for item in outfit_items),
                    ...
                    if occasion in ["party", "wedding", "date", "office_party"]:
                        self.top_history[top_history_key].append(top["id"])
                    combos_added += 1
        ...
        # If still not enough outfits, try less strict matching
        if len(outfits) < 3:
            fallback_outfits = self.recommend_outfits_fallback(occasion, context)
            for outfit in fallback_outfits:
                combo_id = tuple(sorted(item["id"] for item in outfit["items"]))
                if combo_id not in shown_combos:
        ...
        # Update shown combos for this context
    ...
    def recommend_outfits(self, prompt):
        ...
        outfits = self.assemble_outfits(matched_items, required_kws, preferred_kws, forbidden_kws, context, occasion)
    ...
    def recommend_outfits_fallback(self, occasion, context):
        ...
        layers = [i for i in occasion_items if i["category"] == "layer"]
        ...
        if tops and bottoms:
            ...
            for _ in range(min(3, len(tops), len(bottoms))):
                ...
                # Add layer if needed
                if context["needs_layer"] and layers:
    ...
    def generate_outfit_html(self, outfits, filename="outfits.html"):
        ...
        if not outfits:
            ...
            for idx, outfit in enumerate(outfits, 1):
                ...
                html.append(f'<p class="outfit-score">Score: {outfit["score"]:.1f}</p>')
                html.append(f'<p class="outfit-reason">{outfit.get("reason", "")}</p>')
                html.append(f'<p class="color-harmony">Color Harmony: {outfit.get("color_harmony", "")}</p>')
